...
def train(hyp, opt, device, callbacks,mlflow):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            "mAP-.5":results[2],
            "mAP-.5-.95":results[3],
            "val_loss_box":results[4],
            "val_loss_obj":results[5],
            "val_loss_cls":results[6],
            }
...
def main(opt, mlflow, callbacks=Callbacks(), ):
    # Checks
    if RANK in {-1, 0}:
        print_args(vars(opt))
        #check_git_status()
